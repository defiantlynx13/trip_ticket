...
$id = sanitize_text_field($_GET['user_id']);
global $wpdb;
...
if( isset( $_GET['action'] ) && $_GET['action'] == 'delete' && current_user_can('administrator') ){
    if( wp_verify_nonce( $_GET['nonce'] , 'delete') ){
        ...
        if( $del_route ){
            wp_redirect(admin_url(). 'admin.php?page=all-in-passengers&user_id='.$id);
        }
    }
}
...
<div class="wrap">
    ...
    <h1 class="wp-heading-inline">همه زیرمسافران</h1>
    ...
    <table class="wp-list-table widefat fixed striped">
        ...
        <tbody>
            ...
            $get_passengers = $wpdb->get_results( $wpdb->prepare("select * from {$table} where parent=%d and user_id=%s", 1, $id) );
            ...
            if( $wpdb->num_rows > 0 ){
                foreach( $get_passengers as $passengers ):
                    ...
                    echo '<tr>';
                        ...
                        echo '<span><a href="admin.php?page=all-in-passengers&action=delete&&user_id='.$id.'&id='.$passengers->id.'&nonce='.wp_create_nonce('delete').'" onclick="return confirm(\'با حذف مسافر، تمامی زیرمسافران آن نیز حذف می شود. آیا مطمئن هستید؟\')" style="color:red">حذف</a></span>';
                        echo '&nbsp;&nbsp;<span><a href="admin.php?page=edit-in-passenger&action=edit&user_id='.$passengers->user_id.'&id='.$passengers->id.'">ویرایش</a></span>';
                        echo '</div>';
                        echo '</strong></td>';
